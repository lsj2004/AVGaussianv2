...
class AudioFeatureMaskCrossAttention(nn.Module):
    ...
    The public forward contract deliberately matches ``DualBranchAudioUNet``:

    - mono input: ``(B,3,F,T)``
    - difference input: ``(B,2,F,T)``
    - mono and difference masks: ``(B,1,F,T)``

    Visual tokens are supplied out-of-band through :meth:`use_condition`,
    because the upstream AudioGS synthesis function calls its renderer with
    only the two feature tensors.
    """
    ...
    def conditioning_parameters(self) -> list[nn.Parameter]:
        """All parameters are new and belong to the condition baseline."""
        return list(self.parameters())

    def base_parameters(self) -> list[nn.Parameter]:
        """No pretrained U-Net is retained in this renderer."""
        return []
