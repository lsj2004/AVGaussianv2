...
_MARKER = "AVGAUSSIANV2_CROSS_ATTENTION_WORKER_REEXEC"
_ENVIRONMENT = {
    "PYTHONHASHSEED": "42",
...
def _ensure_environment() -> None:
    ...
    if os.environ.get(_MARKER) == "1":
        raise RuntimeError("cross-attention worker deterministic re-exec failed")
    ...
    os.execve(
        ...
        [
            sys.executable,
            "-m",
            "avgaussianv2.cli.benchmark_cross_attention_worker",
            *sys.argv[1:],
        ],
        environment,
    )
...
def main() -> None:
    ...
    evidence = verify_cross_attention_preparation(args.protocol_dir)
    result = run_worker(
